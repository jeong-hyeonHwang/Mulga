...
public class TransactionController {
    ...
    @PostMapping("/combine")
    public ResponseEntity<?> combineTransactions(
        @AuthenticationPrincipal CustomUserDetails userDetails,
        @RequestBody @Valid TransactionCombineDto dto
    ) {
        return ResponseEntity.ok(transactionService.combineTransactions(userDetails, dto));
    }

    @PostMapping("/uncombine/{transactionId}")
    public ResponseEntity<?> combineTransactions(
        @AuthenticationPrincipal CustomUserDetails userDetails,
        @PathVariable String transactionId
    ) {
        return ResponseEntity.ok(transactionService.uncombineTransactions(userDetails, transactionId));
    }
}
